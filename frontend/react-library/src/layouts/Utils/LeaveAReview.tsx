...
export const LeaveAReview: React.FC<{
    submitReview: (starInp: number, reviewDescription: string) => void
}> = (props) => {
    ...
    return <div className='dropdown' style={{ cursor: 'pointer' }}>
        <h5 className='dropdown-toggle' id='dropdownMenuButtonReview' data-bs-toggle='dropdown'>
            Leave a review ?
        </h5>
        <ul id='submitReviewRating' className='dropdown-menu' aria-labelledby='dropdownMenuButtonReview'>
            {/* {Array.from({ length: 10 }).map((_, index) => {
                return <li><button onClick={() => starValue(1 + index * 0.5)} className='dropdown-item'>{1 + index * 0.5} star</button></li>
            })} */}
        ...
        {displayInput === true &&
            <form method='POST' action='#'>
                ...
                <div className="mb-3">
                    ...
                    <textarea className='form-control description-review'
                        rows={3}
                ...
                <div>
                    <button type='button'
                        ...
                        onClick={() => props.submitReview(starInput, reviewDescription)}>Submit Review
                    </button>
                </div>
            </form>
        }
