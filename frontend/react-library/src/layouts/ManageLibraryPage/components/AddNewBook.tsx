...
import AddBookReq from "../../../models/AddBookReq";
...
export const AddNewBook = () => {
    ...
    async function submitNewBook() {
        const url = `${import.meta.env.VITE_API_ADMIN_URL}/secure/add/book`;
        if (authState?.isAuthenticated === true && title !== '' && author !== ''
            && category !== 'Category' && description !== '' && copies >= 0) {
            const book: AddBookReq = new AddBookReq(title, author, description, copies, category);
            ...
            const submitNewBookRes = await fetch(url, reqOptions);
            if (!submitNewBookRes.ok) {
                throw new Error("Some thing went wrong with submit new book !");
            }
            setTitle('');
            setAuthor('');
            setDescription('');
    ...
    return <div className='container mt-5 mb-5'>
        {displaySuccess && <div className='alert alert-success' role='alert'>Book added successfully</div>}
